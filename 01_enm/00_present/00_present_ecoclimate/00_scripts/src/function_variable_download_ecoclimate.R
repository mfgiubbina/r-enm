...
var_download_ecoclimate <- function(baseline, 
                                    scenario, 
                                    variable, 
                                    aogcm, 
                                    path, 
  ...
  # raster options
  raster::rasterOptions(maxmemory = 1e+10, chunksize = 1e+10)
  
  # links dropbox - ecoclimate
  li_dr <- xml2::read_html("https://www.ecoclimate.org/downloads/") %>%
    rvest::html_nodes("a") %>% 
  ...
  for(i in da_do %>% nrow %>% seq){
    ...
    for(j in dir(patt = ".txt")){
      ...
      if(da_do[i, ]$va == "bioclimate"){
        ...
        # prepare data and change col names
        fi <- fi[, -1]

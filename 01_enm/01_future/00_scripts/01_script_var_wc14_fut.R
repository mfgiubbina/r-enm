#' ---
#' title: variables - download, adjust extention, adjust resolution, and correlation to projections of future climate change
...
# directory
path <- "/home/mude/data/github/00_github_organizar/01_r-sdm/00_pragmatico/01_future/01_variables"
setwd(path)
dir()
...
raster::writeRaster(x = var_p_li, 
                    filename = paste0("wc14_li_55km_pres_", names(var_p_li)), 
                    bylayer = TRUE, 
...
raster::writeRaster(x = var_f_li, 
                    filename = paste0("wc14_li_55km_fut_", names(var_f_li)), 
                    bylayer = TRUE, 
                    options = c("COMPRESS=DEFLATE"), 
                    format = "GTiff", 
                    progress = "text",
...
raster::writeRaster(x = var_p_li_sel, 
                    filename = paste0("wc14_55km_pres_", names(var_p_li_sel)), 
                    bylayer = TRUE, 
                    options = c("COMPRESS=DEFLATE"), 
                    format = "GTiff",
                    progress = "text",
...
raster::writeRaster(x = var_f_li_sel, 
                    filename = paste0("wc14_55km_fut_", names(var_f_li_sel)), 
                    bylayer = TRUE, 
                    options = c("COMPRESS=DEFLATE"), 
                    format = "GTiff", 
                    progress = "text",

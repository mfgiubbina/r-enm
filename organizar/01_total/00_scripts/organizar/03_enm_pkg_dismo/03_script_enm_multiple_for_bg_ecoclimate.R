...
# plot
landscapetools::show_landscape(var$ccsm_modern_1950_1999_ne_brazil_pc01) + 
  geom_point(data = occ, aes(x = longitude, y = latitude), size = 2.5, alpha = .7)
...
dir.create("03_enms")
setwd("03_enms")
dir.create("03_enms_ecolimate_pres")
setwd("03_enms_ecolimate_pres")
  ...
  for(r in seq(re)){	# number of replicas
    ...
    train <- dismo::prepareData(x = var, 
                                p = pr_specie %>% 
                                  ...
                                  dplyr::select(longitude, latitude), 
                                b = bg_specie %>% 
                                  dplyr::filter(id %in% bg_sample_train) %>% 
                                  dplyr::select(longitude, latitude)) %>% na.omit

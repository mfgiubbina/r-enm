...
#-------- Frugivores shapefiles (only R. vitellinus --> R. ariel)
ar <- readOGR("C:/Users/User/Desktop/data_0.shp")
plot(ar)
#ar <- spTransform(ar, pres@crs)
...
for (i in 1: length(sp_name)) { 
  ...
  writeRaster(en,  paste0('D:/lilian/Mapas/', gsub(" ", "_", sp_name[i]),"_pres.tif"), format = "GTiff",overwrite=TRUE)
  ...
  # present PA
  writeRaster(pa,  
              paste0('D:/lilian/Mapas/',gsub(" ", "_", sp_name[i]),"_pres_PA.tif"), format = "GTiff",
              overwrite = T)
...
write.csv(evals, "D:/lilian/Mapas/evals_R.ariel.csv")

rownames(vars) <- sp_name
write.csv(vars, 'D:/lilian/Mapas/vars_R.ariel.csv')

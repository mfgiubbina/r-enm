...
## Extrair dados bioclimáticos dos registros de ocorrência

species.var<-extract(clima0k.r,sp[,1:2],cellnumbers=T)
head(species.var)
species.var<-cbind(sp,species.var) ## cbind junta dados coordenadas + bioclimáticos 
head(species.var)


## Identificar os registros de ocorrência duplicados e retirá-los

duplicated(species.var[,3]) ## TRUE para valores duplicados nas células, cells = 3
id<-which(duplicated(species.var[,3])==TRUE) ## cria um arquivo que pergunta (which) de santa.var são TRUE
head(id)
...
## Selecionar background (pseudoausencia)- aleatório
...
back.id<-sample(1:nrow(clima0k),nrow(species.var)) # mantendo prevalência de 0.5
head(back.id)
...
name<-gsub("chaq_","",i) # gsub recorta o nome do arquivo, como eu quero só o nome do modelo e que seja o mesmo para os dados de entrada e dos modelos climaticos fiz assim - pode melhorar!
...
#isto é, TSS é a soma dos acertos dos modelos nas presenças (TPR) e ausências (TNR, descontada a taxa de acertos aleatórios (1).

#A função "evaluate" do pacote dismo calcula TPR e TNR (entre outros índices). Assumimos que temos um objeto de avaliação do modelo bioclim chamado 'Bioclim.eval':
#Bioclim.eval <- evaluate(p= teste[teste[,"pb"]==1, -c(1:3)], a= teste[teste[,"pb"]==0, -c(1:3)], model= Bioclim.model)


#Pra ver todos os índices que a função "evaluate" calcula, basta executar:
#str(Bioclim.eval)

#Pra acessar os índices, use:
#Bioclim.eval@TPR
#Bioclim.eval@TNR
#Bioclim.eval@auc

...
# TRUEhere are some points all the way to the south-east, far from all others. Let"s say we know that this represents a subpopulation that we don"t want to include, and want to remove these points from the analysis. We can find them by first sorting the occs table by latitude.
...
# TRUEhe partitioning method is more clearly illustrated by looking at the background points:
...
d <- pointDistance(bg, centers, lonlat = TRUE)
